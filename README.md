...
此处为界面示意，旧截图中的部分名称可能不同；请以 0.1.3 扩展显示的名称为准。
...
- **BPX 站立**：内部每 0.2 秒发送站立指令并检查运动状态，达到状态 `6` 后才执行下一块。
- **BPX 趴下**：先关闭速度控制，再每 0.2 秒发送趴下指令并检查状态。状态 `3` 表示正在趴下，完全趴下达到状态 `0` 后才执行下一块。
- 两个积木都有 15 秒超时保护；未达到目标状态会报错，不会仅因为等待时间到了就当作动作成功。

因此，不需要在它们外面再套“重复执行”或“重复执行直到”积木。运动结束后，应先停止移动，再执行趴下。
